...
import com.epam.trn.hw5.components.ColorsMenu;
import com.epam.trn.hw5.components.LogsPanel;
import com.epam.trn.hw5.components.Radio;
...
public class DifferentElementsPage extends AbstractPage {
    ...
    public Map<String, String> expectedLog(String logText, WebElement element) {
        ...
        if(element.getText().equals("Water") || element.getText().equals("Wind")
            ...
            map.put(element.getText(), element.getText() + logText);
        }
        else {
            map.put(element.getText(), logText + " " + element.getText());
        }
        return map;
    }
    ...
    public boolean compareCheckboxLogs(WebElement element, String logText) {
        Map<String, String> expected = expectedLog(logText, element);
        List<String> actual = actualLog();
        for(String log : actual) {
            if(log.contains(element.getText())) {
                return log.contains(expected.get(element.getText()));
            }
        }
        return false;
    }
